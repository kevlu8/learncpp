...
Let's continue off from last chapter by including the libraries and defining a main function.
*/
...
int main() {
	...
	To declare a for loop, you need to declare a variable, then specify a condition,
	then specify what to do after each iteration. An example of a valid for loop
	is:
	*/
	...
	executed. In this case, the code inside the loop is std::cout << i << std::endl;
	which prints the value of i to the console, then ends the line. Then, the code
	after the loop is executed, which is i++. This means that i is set to i + 1.
	Then, the condition is checked again. If it is true, the code inside the loop
	is executed again, and so on. Once the condition is false, the loop is exited.
	...
	*/
	...
	If conditions
	=============
	If conditions are used to check if something is true or false. If the condition
	is true, the code inside the if statement is executed. If the condition is
	false, the else if statements are checked. If none of the else if statements
	are true, the else statement is executed. An example of a valid if condition
	is:
	*/
